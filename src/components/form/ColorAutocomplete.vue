...
<template>
  <v-autocomplete label="Color" density="compact" variant="outlined" :items="theme.themeTags.map(({ name }) => name)"
    ...
    <template v-slot:item="{ props, item }">
      <v-list-item v-bind="props" :prepend-icon="`mdi-circle-opacity`"
        :text="`${themeInstance.themes.value.myCustomTheme.colors[item.raw]}`">
        <template #prepend>
          <v-icon icon="mdi-circle-opacity" :color="item.raw"></v-icon>
        </template>
        <v-label>{{ themeInstance.themes.value.myCustomTheme.colors[item.raw] }} </v-label>
      </v-list-item>
    </template>
  </v-autocomplete>
</template>
